#!/usr/bin/env python3
"""
Download ERA5 total precipitation (tp) GRIB files.
...
Variable: total_precipitation (tp, m/day)
Output: {precip_dir}/era5_tp_YYYY_MM_DD.grib
...
def download_single_date(client, date_str, outdir, area=None, verbose=True):
    ...
    target = outdir / f"era5_tp_{safe_date}.grib"
...
def main():
    parser = argparse.ArgumentParser(
        description="Download ERA5 total precipitation (tp) GRIB files"
    )
    ...
    try:
        outdir = Path(get_path(cfg, "precip_dir"))
    except (KeyError, TypeError):
        outdir = Path("data/era5_precip")
    ...
    print("=" * 70)
    print("ERA5 Total Precipitation (tp) — Download Only")
    print("=" * 70)
