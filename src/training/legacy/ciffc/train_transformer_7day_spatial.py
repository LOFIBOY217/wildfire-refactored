"""
Train 7-Day Fire Probability Transformer  (Spatial Position Encoding Version)
==============================================================================
Builds on train_transformer_7day_future.py with two improvements:

1. Default patch_size=8 (vs 16) for finer spatial resolution.
   Memory footprint of meteo_patched stays ~62 GB (inverse P² scaling).

2. Learnable spatial position embedding in the model:
   each patch carries a unique learned position vector so the model can
   learn geographic priors (e.g. western boreal forest risk patterns).

Both training and inference pass the TRUE future meteorological observations
(FWI + 2t + 2d) as decoder input — oracle / upper-bound mode.

Output format (compatible with evaluate_forecast.py):
    outputs/transformer7d_fire_prob_spatial/YYYYMMDD/fire_prob_lead{k:02d}d_YYYYMMDD.tif

Usage:
    python -m src.training.train_transformer_7day_spatial \\
...
from src.models.legacy.transformer_7day_spatial import FireProb7DayTransformerSpatial
...
class Fire7DayDatasetSpatialMixed(Dataset):
    """
    Training dataset: pos_pairs + randomly sampled neg_pairs.
    Returns (x_hist, x_future, y, patch_idx) quadruplets where:
        x_hist    : (in_days,  patch_dim_in)  — historical meteo
        x_future  : (out_days, patch_dim_in)  — future meteo (decoder input)
        y         : (out_days, patch_dim_out) — fire labels
        patch_idx : scalar LongTensor         — spatial patch index
    """
    ...
    def __getitem__(self, idx):
        ...
        x_future = self.meteo[fs:fe, patch_i, :]
        y        = self.fire[fs:fe,  patch_i, :]
        return (torch.from_numpy(x.copy()),
                torch.from_numpy(x_future.copy()),
                torch.from_numpy(y.copy()),
                torch.tensor(patch_i, dtype=torch.long))


class Fire7DayDatasetSpatialPrecompute(Dataset):
    """
    Unfiltered val dataset — evaluates on all patches for honest AUC estimate.
    Returns (x_hist, x_future, y, patch_idx) quadruplets.
    """
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Train 7-Day Fire Transformer (Spatial Position Encoding + Future Meteo)"
    )
    ...
    ap.add_argument("--patch_size",   type=int,   default=8,
                    help="Spatial patch size in pixels (default=8). "
                         "Smaller patches preserve more spatial detail.")
    ...
    output_dir = os.path.join(get_path(cfg, "output_dir"),
                              "transformer7d_fire_prob_spatial")
    ckpt_dir   = os.path.join(get_path(cfg, "checkpoint_dir"),
                              "transformer_7day_spatial")
    ...
    print("\n" + "=" * 70)
    print("7-DAY FIRE PROBABILITY TRANSFORMER  [Spatial + Future Meteo]")
    print("=" * 70)
    ...
    print(f"  patch_size        : {args.patch_size}  (pixels per patch edge)")
    ...
    print(f"  patch_size={P}  grid={grid[0]}x{grid[1]}  n_patches={n_patches:,}")
    print(f"  in_dim={in_dim}  out_dim={out_dim}")
    print(f"  spatial_embed size: {n_patches} × {args.d_model} = "
          f"{n_patches * args.d_model * 4 / 1e6:.1f} MB")
    ...
    # ----------------------------------------------------------------
    train_ds = Fire7DayDatasetSpatialMixed(
        meteo_patched, fire_patched, train_wins, hw, grid, all_pairs
    )
    if val_wins:
        val_ds = Fire7DayDatasetSpatialPrecompute(
    ...
    model = FireProb7DayTransformerSpatial(
    ...
    print(f"  Parameters: {n_params:,}  "
          f"(spatial_embed: {n_patches * args.d_model:,})")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        model.train()
        train_loss = 0.0
        for xb, xfb, yb, pidx in train_dl:
            xb   = xb.to(device)
            xfb  = xfb.to(device)
            yb   = yb.to(device)
            pidx = pidx.to(device)
            logits = model(xb, xfb, pidx)
        ...
        val_loss = 0.0
        with torch.no_grad():
            for xb, xfb, yb, pidx in val_dl:
                xb   = xb.to(device)
                xfb  = xfb.to(device)
                yb   = yb.to(device)
                pidx = pidx.to(device)
                val_loss += criterion(model(xb, xfb, pidx), yb).item() * xb.size(0)
        val_loss /= len(val_ds)
    ...
    for base_date in pred_dates:
        ...
        if base_idx < in_days:
            continue
        if base_idx + out_days > T:
            print(f"  [SKIP] {base_date}: future meteo not available")
            continue

        hist   = meteo_norm[base_idx - in_days : base_idx]
        future = meteo_norm[base_idx : base_idx + out_days]

        xp, pred_hw, pred_grid = patchify(hist, P)
        fp, _,       _         = patchify(future, P)
