"""
Train 7-Day Fire Probability Transformer  (Future Meteo Decoder Version)
=========================================================================
Variant of train_transformer_7day_posaware.py where the decoder receives the
actual future 7-day meteorological observations (FWI + 2m temperature +
2m dewpoint) as input, instead of learnable lead-time queries.

This is an oracle / upper-bound experiment: both training and inference use
the TRUE future weather as decoder input.  When S2S ECMWF forecast data
becomes available the same interface accepts forecast values.

Mixed Patch Sampling (inherited from posaware version):
...
Output format (compatible with evaluate_forecast.py):
    outputs/transformer7d_fire_prob_future/YYYYMMDD/fire_prob_lead{k:02d}d_YYYYMMDD.tif

Usage:
    python -m src.training.train_transformer_7day_future \\
...
from src.models.legacy.transformer_7day_with_future import FireProb7DayTransformerWithFuture
...
class Fire7DayDatasetFutureMixed(Dataset):
    """
    Training dataset: pos_pairs + randomly sampled neg_pairs.
    Returns (x_hist, x_future, y) triplets where:
    ...
    def __getitem__(self, idx):
        ...
        x_future = self.meteo[fs:fe, patch_i, :]   # (out_days, patch_dim_in)
        y        = self.fire[fs:fe,  patch_i, :]   # (out_days, patch_dim_out)
        return (torch.from_numpy(x.copy()),
                torch.from_numpy(x_future.copy()),
                torch.from_numpy(y.copy()))


class Fire7DayDatasetFuturePrecompute(Dataset):
    """
    Unfiltered val dataset — evaluates on all patches for honest AUC estimate.
    Returns (x_hist, x_future, y) triplets.
    """
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Train 7-Day Fire Probability Transformer (Future Meteo Decoder)"
    )
    ...
    output_dir = os.path.join(get_path(cfg, "output_dir"),
                              "transformer7d_fire_prob_future")
    ckpt_dir   = os.path.join(get_path(cfg, "checkpoint_dir"),
                              "transformer_7day_future")
    ...
    print("\n" + "=" * 70)
    print("7-DAY FIRE PROBABILITY TRANSFORMER  [Future Meteo Decoder]")
    print("=" * 70)
    ...
    # ----------------------------------------------------------------
    train_ds = Fire7DayDatasetFutureMixed(
        meteo_patched, fire_patched, train_wins, hw, grid, all_pairs
    )
    if val_wins:
        val_ds = Fire7DayDatasetFuturePrecompute(
    ...
    model = FireProb7DayTransformerWithFuture(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_loss = 0.0
        with torch.no_grad():
            for xb, xfb, yb in val_dl:
                xb  = xb.to(device)
                xfb = xfb.to(device)
                yb  = yb.to(device)
                val_loss += criterion(model(xb, xfb), yb).item() * xb.size(0)
        val_loss /= len(val_ds)
    ...
    for base_date in pred_dates:
        ...
        if base_idx + out_days > T:
            print(f"  [SKIP] {base_date}: future meteo not available (need {out_days} more days)")
            continue

        hist   = meteo_norm[base_idx - in_days : base_idx]       # (7, H, W, 3)
        future = meteo_norm[base_idx : base_idx + out_days]      # (7, H, W, 3)
