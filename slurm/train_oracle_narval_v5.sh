...
#SBATCH --output=/scratch/jiaqi217/logs/train_oracle_v5_%j.out
#SBATCH --error=/scratch/jiaqi217/logs/train_oracle_v5_%j.err
...
# oracle v5 architecture enhancements on top of v4's batch_size fix:
#   1. batch_size=1024: 8x more gradient updates/epoch (same epoch wall time, IO-bound)
#   2. --use_patch_embed: learnable 256-dim spatial embedding per patch (~6M params)
#      → model learns location-specific fire patterns beyond fire_clim scalar
#   3. --mlp_dec_embed: 2-layer MLP for decoder (2048→1024→256 vs 2048→256 linear)
#      → better preserves oracle ERA5 feature structure when compressing to d_model
# Combined, these address: gradient shortage + spatial blindness + embedding bottleneck
ts "=== STARTING TRAINING (oracle v5 — batch=1024 + patch_embed + mlp_dec_embed) ==="
$PYTHON src/training/train_s2s_hotspot_cwfis_v2.py \
  --config configs/paths_narval.yaml \
  --run_name oracle_narval_v5 \
  --decoder oracle \
  --pred_end 2025-10-31 \
  --num_workers 8 \
  --batch_size 1024 \
