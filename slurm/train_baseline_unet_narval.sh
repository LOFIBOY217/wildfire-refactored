#!/bin/bash
#SBATCH --job-name=wf-unet-baseline
#SBATCH --gpus-per-node=1
#SBATCH --time=0-12:00:00
#SBATCH --mem=400G
#SBATCH --output=/scratch/jiaqi217/logs/unet_baseline_%j.log
#SBATCH --error=/scratch/jiaqi217/logs/unet_baseline_%j.err
#SBATCH --account=def-inghaw
#SBATCH --mail-type=FAIL
#SBATCH --mail-user=jiaaqii.huang@mail.utoronto.ca

# ----------------------------------------------------------------
# U-NET BASELINE — 12y × 9ch
# ----------------------------------------------------------------
# The CNN / segmentation family (canonical burned-area architecture).
# Answers: "does multi-scale spatial modeling beat recurrence/attention?"
# Fully-convolutional U-Net over one 16x16 patch: 21-day history folded
# into channels, 2 downsampling stages (16->8->4) with skip connections,
# per-patch decoder-mean broadcast-added (same conditioning as ConvLSTM).
#
# Range / channels / hyperparams IDENTICAL to the ConvLSTM baseline for a
# fair same-setup family comparison (12y_2014, 9ch, batch 2048, 4 epochs).
# ----------------------------------------------------------------
...
RUN_NAME="baseline_unet_12y_2014_9ch"
...
$PYTHON -u -m src.training.train_v3 \
    ...
    --batch_size 2048 --epochs 4 --lr 1e-4 --weight_decay 0.01 --dropout 0.2 \
    ...
    --wandb_tags "baseline,unet,9ch,12y_2014" \
