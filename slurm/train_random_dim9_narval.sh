...
# Hypothesis: random dim=9 ≈ random dim=2048 → decoder dimension doesn't matter for random noise
...
if [ -n "$PF_IN_SCRATCH" ]; then
    ...
    ts "pf.dat not in SCRATCH_CACHE → building in SCRATCH_CACHE"
fi

ts "=== STARTING TRAINING (random decoder, dec_dim=9 ablation) ==="
$PYTHON src/training/train_s2s_hotspot_cwfis_v2.py \
  --config configs/paths_narval.yaml \
  --run_name random_dim9_ablation \
  --decoder random \
  --dec_dim 9 \
  --pred_end 2025-10-31 \
  --num_workers 8 \
