...
#SBATCH --job-name=wf-tp-2023-2025
#SBATCH --time=8:00:00
#SBATCH --mem=32G
...
# ----------------------------------------------------------------
# Process tp 2023-2025 GRIBs → daily TIF on FWI grid (EPSG:3978)
#
# Input:  data/era5_precip/era5_tp_2023/2024/2025_*.grib  (1035 files)
# Output: data/era5_precip/tp_YYYYMMDD.tif
# Skips already-existing TIFs automatically.
# ----------------------------------------------------------------
...
echo "============================================="
echo "  Process tp 2023-2025 GRIBs → FWI grid TIFs"
...
grib_dir = 'data/era5_precip'
# Only 2023-2025 GRIBs
gribs = sorted(
    glob.glob(os.path.join(grib_dir, 'era5_tp_2023_*.grib')) +
    glob.glob(os.path.join(grib_dir, 'era5_tp_2024_*.grib')) +
    glob.glob(os.path.join(grib_dir, 'era5_tp_2025_*.grib'))
)
print(f'Found {len(gribs)} tp 2023-2025 GRIB files')
...
for gi, gf in enumerate(gribs):
    ...
    out_path = os.path.join(grib_dir, f'tp_{date_str}.tif')
    ...
    if (gi + 1) % 100 == 0:
        print(f'  tp: processed={done} skipped={skip} failed={fail} / {gi+1}')

print(f'\nDONE: processed={done} skipped={skip} failed={fail} total={len(gribs)}')
n_total = len(glob.glob('data/era5_precip/tp_*.tif'))
first = sorted(glob.glob('data/era5_precip/tp_*.tif'))[0] if n_total else 'N/A'
last  = sorted(glob.glob('data/era5_precip/tp_*.tif'))[-1] if n_total else 'N/A'
print(f'Total tp TIFs on disk: {n_total}  [{os.path.basename(first)} -> {os.path.basename(last)}]')
" 2>&1
