#!/bin/bash
#SBATCH --job-name=wf-v3-16ch-2000
#SBATCH --gpus-per-node=1
#SBATCH --time=3-00:00:00
#SBATCH --mem=500G
#SBATCH --output=/scratch/jiaqi217/logs/train_v3_16ch_2000_%j.log
#SBATCH --error=/scratch/jiaqi217/logs/train_v3_16ch_2000_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=jiaaqii.huang@mail.utoronto.ca
#SBATCH --account=def-inghaw

# ----------------------------------------------------------------
#   V3 16ch x enc{ENC} x 2000-2025 EXTENDED TRAINING
# ----------------------------------------------------------------
# Channel set: 13ch + u10 + v10 + CAPE  (full feature set)
# CHANNELS = FWI,2t,fire_clim,2d,tcw,sm20,deep_soil,precip_def,
#            u10,v10,CAPE,NDVI,population,slope,burn_age,burn_count
#
# Compares to 9ch SOTA (enc21 = 6.47x) and 13ch sweep — does adding
# wind + atmospheric instability give the final boost?
#
# Usage:
#   ENC=14 sbatch slurm/train_v3_16ch_2000_narval.sh
#   ENC=21 sbatch slurm/train_v3_16ch_2000_narval.sh
#   ENC=28 sbatch slurm/train_v3_16ch_2000_narval.sh
#   ENC=35 sbatch slurm/train_v3_16ch_2000_narval.sh
#
# PREREQUISITES:
...
#   - u10/v10/cape clean (post nan-edge bug fix). Verified 2026-04-19:
#     all sampled files at 0.00% nan.
#   - 16ch cache built at $SCRATCH/meteo_cache/v3_full_2000
#     run: CACHE_CHANNELS=16 sbatch slurm/rebuild_cache_2000_2025_narval.sh
# ----------------------------------------------------------------
...
CACHE_DIR_2000="$SCRATCH/meteo_cache/v3_full_2000"
RUN_NAME="v3_16ch_enc${ENC}_2000"

if [ ! -d "$CACHE_DIR_2000" ] || [ -z "$(ls -A "$CACHE_DIR_2000" 2>/dev/null)" ]; then
    echo "ERROR: $CACHE_DIR_2000 is empty or missing."
    echo "Run cache build first: CACHE_CHANNELS=16 sbatch slurm/rebuild_cache_2000_2025_narval.sh"
    exit 1
fi

echo "============================================="
echo "  V3 16ch x enc${ENC} x 2000-2025 (full feature set)"
