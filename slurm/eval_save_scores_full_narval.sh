...
# ----------------------------------------------------------------
# FULL eval (--full_val): save per-pixel scores on ALL val windows
# (vs 20-window sample). Required for paper-quality scaling-law plot
# and per-month / per-region stratification.
#
# Usage:
#   RANGE=4y  ENC=14 sbatch slurm/eval_save_scores_full_narval.sh
#   RANGE=4y  ENC=21 sbatch slurm/eval_save_scores_full_narval.sh
#   RANGE=4y  ENC=28 sbatch slurm/eval_save_scores_full_narval.sh
#   RANGE=4y  ENC=35 sbatch slurm/eval_save_scores_full_narval.sh
#   RANGE=22y ENC=14 sbatch slurm/eval_save_scores_full_narval.sh
#   RANGE=22y ENC=21 sbatch slurm/eval_save_scores_full_narval.sh
#   RANGE=22y ENC=28 sbatch slurm/eval_save_scores_full_narval.sh
#   RANGE=22y ENC=35 sbatch slurm/eval_save_scores_full_narval.sh
# ----------------------------------------------------------------
...
case "$RANGE" in
    22y) DATA_START=2000-05-01; CACHE_LUSTRE=v3_9ch_2000;     RUN_TAG=2000 ;;
    12y) DATA_START=2014-05-01; CACHE_LUSTRE=v3_9ch_12y_2014; RUN_TAG=12y_2014 ;;
...
echo "  --full_val (ALL windows, not 20-win sample)"
echo "  ckpt: $CKPT"
echo "  out : $SCORES_DIR"
echo "============================================="
